...
def init_db():
    ...
    samples = [
        ...
        (r"関数 $f(x) = x^3 - 3x^2 + 4$ の極小値を求めよ。", "0", 60),
        (r"極限 $\lim_{n \to \infty} \frac{1}{n^2} \sum_{k=1}^n k$ の値を求めよ。（入力例：1/3）", "1/3", 60),
        (r"方程式 $\log_2(x) + \log_2(x-3) = 2$ を解け。", "4", 60),
        (r"定積分 $\int_0^1 x^2 e^x \, dx$ の値を求めよ。（入力例：e-2）", "e-2", 90),
        (r"初項 $2$、公差 $3$ の等差数列の第10項を求めよ。", "29", 45),
        (r"ベクトル $\vec{a}=(2, 1)$, $\vec{b}=(1, 3)$ のなす角 $\theta$ を求めよ。（$0^\circ \le \theta \le 180^\circ$）", "45", 60),
        (r"方程式 $z^3 = 1$ の虚数解のうち、虚部が正であるものを $\omega$ とするとき、$\omega^2 + \omega + 1$ の値を求めよ。", "0", 60),
        ...
        (r"0 \le x \le \pi において、関数 $f(x) = \sin 3x + \cos 3x$ の最大値を求めよ。", "1", 90),
        ...
        (r"極限 $\lim_{x \to 0} \frac{(2x - 1 - 2x)(x2)}{x}$ の値を求めよ。", "2", 90),
        (r"方程式 $z^2 = -1$ の解のうち、複素数平面上で第1象限にあるものを求めよ。（(1+i)/sqrt(2)）", "(1+i)/sqrt(2)", 120),
        (r"放物線 $y = x^2$ 上の点 $(1, 1)$ における接線と、この放物線で囲まれた部分の面積を求めよ。（入力例：4/3）", "4/3", 90),
        (r"定積分 $\int_0^{\pi/2} \sin 3x \cdot \cos 3x \, dx$ の値を求めよ。（入力例：2/3）", "2/3", 60),
